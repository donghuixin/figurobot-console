...
- 🛡️ **上电安全判断** — 自动评估当前姿态是否安全，识别「电压错误 / 超限 / 离线」等状态
...
舵机报 `0x10 Input Voltage Error`（输入电压错误）时，会拒绝所有读取指令、只响应 Ping，表现为「全部离线」。

- **舵机工作电压：6.0V – 8.4V（标称 7.4V，2S 锂电池）**
- ⚠️ **切勿过压**（>8.4V 会永久损坏舵机）
- 请检查：电池电量 / 电源适配器规格 / 电源线接触
